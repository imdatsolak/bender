...
TODO: There is still so much to do here. Sometimes, it doesn't detect correctly certain dates, which
      means we still need to review the order of our regex's in the regex-array :-)
"""
...
date_detectors = [
        ...
        { 
            'regex': re.compile(r'[0-3]{0,1}[0-9].[ ]+\b(januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember|jan|feb|mär|mar|apr|mai|jun|jul|aug|sep|okt|nov|dez)\b[ ,]{0,1}[2]{0,1}[0]{0,1}[1-3][0-9]'),
            'pre_code': lambda entry: utcize_date(entry)
        },
        # Detects [0]1. Januar
        {
            'regex': re.compile(r'[0-3]{0,1}[0-9].[ ]+\b(januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember|jan|feb|mär|mar|apr|mai|jun|jul|aug|sep|okt|nov|dez)\b'),
...
class MLDateTimePhoneDetector:
    PIT = 'PIT'
    PIT_NOW = 'PITNOW'
    PIT_NOW_SPECIAL = 'PITNOWSPECIAL'
    TIME_FRAME = 'TIMEFRAME'
    PHONE = 'PHONE'
    PIT_PAST = 'PITPAST'
    PIT_FUTURE = 'PITFUTURE'
...
"""
This is now the enricher. It is just a short-cut to do conversion of detected dates and times into
a more 'machine-readable'-format...
"""
