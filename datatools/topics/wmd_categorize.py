...
def analyze_directory(indir, jsonFile, w2v_file, outfile, verbose, outdir, numthreads, lang, remove_stop_words, speller=None, stemmer=None):
    try:
...
def usage():
    print('Usage:')
    print('\tpython wmd_categorize.py -i|--indir <in-directory> -j|--json <jsonFile> -w|--w2v <word2vec-file> [-v] [-o|-outfile <outfile>] [-O|--outdir <dir-to-copy-files-to>] [--speller <hun|enchant>] [--stem] [--numthreads <n>]')
    print()
    print('\t-i <indir>         Directory containing .json-email-files')
    print('\t-j <json-file>     File containing category descriptions')
...
if __name__ == '__main__':
    try:
        options, arguments = getopt.getopt(sys.argv[1:], 'i:j:w:vo:O:', ['indir=', 'jsonFile=', 'w2v=', 'verbose', 'outfile=', 'outdir=', 'numthreads=', 'speller=', 'stem'])
    ...
    if indir is None or jsonFile is None or w2v_file is None:
        usage()
    ...
    analyze_directory(indir, jsonFile, w2v_file, outfile, verbose, outdir, numthreads, language_short, remove_stop_words, speller=speller, stemmer=stemmer)
